...
def OLD_correct_lin_trend_phase(Y, resol):
    ...
    size = len(Y)//resol 
    coef_lin_left_by_chunk = [np.polyfit(np.linspace(j*size,(j+1)*size,size), Y[j*size:(j+1)*size], 1)[0] for j in range(len(Y)//3//size)]
    coef_lin_right_by_chunk = [np.polyfit(np.linspace(len(Y)-(j+1)*size, len(Y)-j*size,size), Y[len(Y)-(j+1)*size:len(Y)-j*size], 1)[0] for j in range(len(Y)//3//size)]
    a = coef_lin_left_by_chunk.copy() + coef_lin_right_by_chunk.copy() 
    lin_trend = sum(a) / len(a)
    Y_cor = Y.copy() - lin_trend*(x_cor - x_cor[len(Y)//2])                    
    plt.figure()  
...
def frame_S_param_phase(Y, tol = 0.1):
    X = np.arange(len(Y))
    Y_s = savgol_filter(Y, 11, 2)
    d2y_dx2 = np.gradient(np.gradient(Y_s, X), X)
    ...
    # plt.subplot(2, 1, 2) 
    # plt.plot(d2y_dx2, 'g--')
    # plt.plot(peaks_idxs, [d2y_dx2[i] for i in peaks_idxs], 'b*')
    # plt.show()
...
##################################################
            ...
            try:    
                ...
                # Y_framed = OLD_frame_S_param_phase(Y, size = 10, tol = 0.01)                   
            except Exception as e:
                print(e)
                Y_cor = correct_lin_trend_phase(Y)
                # Y_cor = OLD_correct_lin_trend_phase(Y, resol = 50)
                Y_framed = frame_S_param_phase(Y_cor)                   

            # Y_cor_framed = correct_lin_trend_phase(Y_framed)
            Y_cor_framed = OLD_correct_lin_trend_phase(Y, resol = 50)

            popt = fano_fitting(X,Y_cor_framed)
