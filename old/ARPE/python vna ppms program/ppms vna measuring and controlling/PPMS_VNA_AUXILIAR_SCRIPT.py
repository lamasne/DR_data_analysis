...
def vna_multimode(freq1, ratio21, ratio41, event):
    ...
    ZNA.write(':SENS1:FREQ:CENT ' + str(
        freq1) + ' GHz' + ";*OPC")  # setting center and span of the signal that vna is emiting
    ...
    ZNA.write(':SOUR1:POW -10 dBm' + ";*OPC")  # power (can be written as a variable like it is the first frequency)
    ...
    ZNA.write(
        ':DISP:TRAC:Y:AUTO ONCE, \'Trc1\'' + ";*OPC")  # autoscale traces (this command can be copy-pasted in other places of this script if it is necessary)
    ...
    ZNA.write(':SENS2:FREQ:CENT ' + str(
        round(multifreq()[0])) + ' Hz' + ";*OPC")  # we set the new freq. mode in function of the first peak
    ZNA.write(':SENS2:FREQ:SPAN 100000 kHz' + ";*OPC")
    ZNA.write(':SENS2:SWE:POIN 401' + ";*OPC")
    ZNA.write('SENS2:BAND 1 KHz' + ";*OPC")
    ZNA.write(':SOUR2:POW -10 dBm' + ";*OPC")
    ...
    ZNA.write(':SENS3:FREQ:CENT ' + str(
        round(multifreq()[1])) + ' Hz' + ";*OPC")  # we set the new freq. mode in function of the first peak
    ZNA.write(':SENS3:FREQ:SPAN 100000 kHz' + ";*OPC")
    ZNA.write(':SENS3:SWE:POIN 401' + ";*OPC")
    ZNA.write('SENS3:BAND 1 KHz' + ";*OPC")
    ZNA.write(':SOUR3:POW -10 dBm' + ";*OPC")
    ...
    # avaluating some parameters before starting the measuring loop
    ...
    while True:
        ...
        # getting field and temperature just before taking the data
        ...
        ZNA.write(
            'MMEM:STOR:TRAC "TRC3", "C:/Users/Public/Documents/Rohde-Schwarz/VNA/Joan/ ' + str(n)+ str('_1_') + str(
                user_temperature) + str('_') + str(system_temperature) + str('_') + str(qd.getField()) + str(
                '_.s2p') + str("\"") + ";*OPC")
        ZNA.write(
            'MMEM:STOR:TRAC "TRC7", "C:/Users/Public/Documents/Rohde-Schwarz/VNA/Joan/ ' + str(n)+ str('_2_') + str(
                user_temperature) + str('_') + str(system_temperature) + str('_') + str(qd.getField()) + str(
                '_.s2p') + str(
                "\"") + ";*OPC")
        ZNA.write(
            'MMEM:STOR:TRAC "TRC11", "C:/Users/Public/Documents/Rohde-Schwarz/VNA/Joan/ ' + str(n)+ str('_3_') + str(
                user_temperature) + str('_') + str(system_temperature) + str('_') + str(qd.getField()) + str(
                '_.s2p') + str(
                "\"") + ";*OPC")

        n=n+1
        time.sleep(0.5)  # time between measurements (below a minimum time it gives an error), every measurement it takes also arround 2 secs

        #connection between the main and the auxiliar scripts
        if event.is_set():
            break
        time.sleep(3)
